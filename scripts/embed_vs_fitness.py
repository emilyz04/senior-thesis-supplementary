...
# Global setup
# Force JAX/EvoFR to CPU so it does not compete with ESM for GPU VRAM
...
# Global variables for model (will be loaded once)
_tokenizer = None
_model = None
_device = None

def load_ESM_model():
    """Load ESM model and tokenizer once globally"""
    global _tokenizer, _model, _device

    if _tokenizer is None or _model is None:
        print("Loading ESM model...")
        _device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        model_path = "/home/ez1199/pretrained_model/esm2_local"
        _tokenizer = AutoTokenizer.from_pretrained(model_path, local_files_only=True)
        _model = AutoModel.from_pretrained(model_path, local_files_only=True).to(_device)
        _model.eval()

        if _device == torch.device("cpu"):
            _model.to(torch.float32)
        print(f"ESM model loaded on {_device}")

    return _tokenizer, _model, _device

def cleanup_ESM_model():
    """Clean up ESM model from memory"""
    global _tokenizer, _model, _device
    ...
    gc.collect()
    print("ESM model cleaned up from memory")

def analyze_lineage_fitness(parent_lineage, lineages, start_date, end_date, protein="surface glycoprotein"):
    """
    Analyze the relationship between protein embedding distances and fitness differences
    ...
    Returns:
        pd.DataFrame: Results containing embedding distances and fitness differences
    """
    
    print(f"Processing {parent_lineage} from {start_date} to {end_date}")
    
    # Use pre-loaded model
    tokenizer, model, device = load_ESM_model()
    
    try:
        # === PROTEIN EMBEDDING SECTION ===
        sequences = []
        sequence_ids = []
        files = [
        ...
        for file in files:
            ...
            for record in SeqIO.parse(file, "fasta"):
                ...
                if lineage not in lineage_to_first_sequence:
                    lineage_to_first_sequence[lineage] = cleaned_seq
        ...
        def batch_iter(seqs, ids, batch_size):
            for i in range(0, len(seqs), batch_size):
                yield seqs[i:i + batch_size], ids[i:i + batch_size]

        batch_size = 8
        all_embeddings = []
        batched_ids = []

        for batch_seqs, batch_ids in batch_iter(sequences, sequence_ids, batch_size):
            tokens = tokenizer(batch_seqs, add_special_tokens=True, padding="longest", return_tensors="pt")
            ...
            for i in range(input_ids.size(0)):
                emb = outputs.last_hidden_state[i, 1:attention_mask[i].sum()-1]
                mean_emb = emb.mean(dim=0).cpu()
                all_embeddings.append(mean_emb)
                batched_ids.append(batch_ids[i])

            # Memory cleanup
            del input_ids, attention_mask, outputs, tokens
            torch.cuda.empty_cache()

        # Group embeddings by lineage
        lineage_to_embeddings = defaultdict(list)
        for emb, lin in zip(all_embeddings, batched_ids):
            lineage_to_embeddings[lin].append(emb)

        # Clear sequences to free memory
        del sequences, sequence_ids, all_embeddings, batched_ids
        gc.collect()
        ...
        if parent_lineage not in lineage_to_embeddings:
            print(f"Parent lineage {parent_lineage} not found in embeddings. Skipping.")
            return pd.DataFrame()

        ref_emb = lineage_to_embeddings[parent_lineage][0]
        parent_seq = lineage_to_first_sequence.get(parent_lineage)

        child_lineages = []
        embedding_distances = []

        for lineage, embeddings in lineage_to_embeddings.items():
            if lineage == parent_lineage:
                continue
            child_seq = lineage_to_first_sequence.get(lineage)
            ...
            dist = l1_distance(ref_emb, child_emb)
            child_lineages.append(lineage)
            embedding_distances.append(dist)

        # Clear embedding data
        del lineage_to_embeddings, lineage_to_first_sequence, ref_emb
        gc.collect()
        ...
        raw_seq["sequences"] = raw_seq["sequences"].astype(np.int32)
        ...
        paired = [(lin, dist) for lin, dist in zip(child_lineages, embedding_distances)
        ...
        if not paired:
            ...
            return pd.DataFrame()
        child_lineages, embedding_distances = zip(*paired)

        result_df = pd.DataFrame({
            "Parent Lineage": [parent_lineage] * len(child_lineages),
            "Child Lineage": list(child_lineages),
            "Embedding Distance": list(embedding_distances)
        })
        ...
        valid_x = result_df.dropna(subset=["Embedding Distance", "Fitness Difference"])["Embedding Distance"].astype(float).to_numpy()
        if len(np.unique(valid_x)) <= 1:
            print(f"Insufficient x variation for regression: {parent_lineage} {start_date}-{end_date}. Skipping.")
            return pd.DataFrame()

        # Save results
        output_dir = "spikesummary_test"
        os.makedirs(output_dir, exist_ok=True)
        filename = f"summary_{parent_lineage}_{start_date}to{end_date}_{protein.replace(' ', '_')}.csv"
    ...
    # Note: Model cleanup is handled separately via cleanup_ESM_model()


def create_fitness_plot(result_df, parent_lineage, start_date, end_date, protein):
    """Create and save the fitness vs embedding distance plot"""
    
    # Clean data
    result_df["Embedding Distance"] = pd.to_numeric(result_df["Embedding Distance"], errors='coerce')
    ...
    clean_df = result_df.dropna(subset=["Embedding Distance", "Fitness Difference", "Child Lineage", "Parent Lineage"])

    if len(clean_df) == 0:
        print("No valid data for plotting")
        return

    x = clean_df["Embedding Distance"].astype(float).to_numpy()
    ...
    plt.xlabel("L1 Embedding Distance")
    plt.ylabel("Log Relative Fitness Change")
    plt.title(f"Log Relative Fitness Change vs L1 Embedding Distance\n{start_date} to {end_date}, {parent_lineage} Parent Lineage, {protein}", fontsize=10)
    plt.tight_layout()

    output_dir = "spikeplots_test"
    os.makedirs(output_dir, exist_ok=True)
    filename = f"plot_{parent_lineage}_{start_date}to{end_date}_{protein.replace(' ', '_')}.png"
...
if __name__ == "__main__":
    ...
    try:
        ...
        if all_results:
            ...
            combined_results.to_csv("combined_lineage_analysis.csv", index=False)

...
folder_path = "/home/ez1199/spikesummary_test"
distance = "embed"
output_dir = "/home/ez1199/test/covariance_embedding"
...
for filename in os.listdir(folder_path):
    if filename.endswith(".csv"):
        ...
        # Expected format: summary_PARENT_STARTtoEND_surface_glycoprotein.csv
        try:
            base = filename.replace("summary_", "").replace("_surface_glycoprotein.csv", "")
...
combined_df["Embedding Distance"] = pd.to_numeric(combined_df["Embedding Distance"], errors="coerce")
combined_df["Fitness Difference"] = pd.to_numeric(combined_df["Fitness Difference"], errors="coerce")
combined_df = combined_df.dropna(subset=["Embedding Distance", "Fitness Difference"])
combined_df = combined_df[
    np.isfinite(combined_df["Embedding Distance"]) & np.isfinite(combined_df["Fitness Difference"])
]
...
for min_n in thresholds:
    ...
    ancova_df = filtered_df.rename(columns={
        "Embedding Distance": "Emb_Dist",
        "Fitness Difference": "Fit_Diff"
    })

    ancova_model = smf.ols(
        "Fit_Diff ~ Emb_Dist * C(Group, Sum)",
        data=ancova_df
    ).fit()
    ...
    emb_terms = summary_table.filter(like="Emb_Dist", axis=0)
    print("\nEmbedding Distance Coefficients:")
    print(emb_terms.to_string())
    ...
    ancova_summary = pd.DataFrame({
        ...
        "Value": [min_n, int(ancova_model.nobs), filtered_df["Group"].nunique(),
                  ...
                  round(emb_terms["Coef."].iloc[0], 4),
                  round(emb_terms["P>|t|"].iloc[0], 4)]
    })
    ancova_summary.to_csv(os.path.join(output_dir, f"{distance}_ancova_summary_n{min_n}.csv"), index=False)
    emb_terms.to_csv(os.path.join(output_dir, f"{distance}_ancova_results_n{min_n}.csv"))
    ...
    for group, subset in filtered_df.groupby("Group"):
        x = subset["Embedding Distance"].to_numpy()
        ...
        if len(np.unique(x)) <= 1:
            print(f"Skipping {group}: all embedding distances are identical")
            continue
